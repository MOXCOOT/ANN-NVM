...
class FixedDegreeGraph : public Graph
{
  private:
    ...
    dist_t distance( size_t a, T& b )
    {
        ...
        else if ( dist_type == DistType::NEGATIVE_INNER_PROD )
            return data->negative_inner_prod_distance( a, b );
        else if ( dist_type == DistType::NEGATIVE_COSINE )
    ...
    void compute_distance( size_t offset, std::vector< dist_t >& dists ) { compute_distance_naive( offset, dists ); }
  ...
  public:
    pobj::p< long long > total_explore_cnt = 0;
    pobj::p< int > total_explore_times = 0;
    ...
    void astar_multi_start_search( const std::vector< std::pair< int, value_t > >& query, int k, std::vector< size_t >& result )
    {
    ...
    void search_top_k( const std::vector< std::pair< int, value_t > >& query, int k, std::vector< size_t >& result ) { astar_multi_start_search( query, k, result ); }
};